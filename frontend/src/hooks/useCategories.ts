...
export default function useCategories(
  ...
  const handleCategoryChange = useCallback(async (catId: string, field: string, value: any) => {
    ...
    if (updatedCat.name && updatedCat.name.trim() !== '') {
      const toSave = {
        ...
        cashflow_group_id: updatedCat.cashflowGroup || updatedCat.cashflow_group_id,
  ...
  const handleDeleteCategory = useCallback(
    async (id: string, transactions: any[]) => {
  ...
  const handleMoveCategory = useCallback(
    async (id: string, direction: string) => {
      ...
      if (swapIndex !== -1) {
        ...
        const toBackend = (c: Category) => ({
          id: c.id,
          name: c.name,
          icon: c.icon,
          color: c.color,
          cashflow_group_id: c.cashflowGroup || c.cashflow_group_id,
