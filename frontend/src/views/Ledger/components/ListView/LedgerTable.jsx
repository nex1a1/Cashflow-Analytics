    ...
    return (
      <th
        className={`px-4 py-3 font-bold cursor-pointer transition-all select-none group text-${align} ${className} ${
          `text-slate-400 hover:text-slate-200 ${isActive ? 'text-blue-400 bg-slate-900/60' : 'hover:bg-slate-850/40'}`
        }`}
      ...
      >
        <div className={`inline-flex items-center gap-1.5 text-xs uppercase tracking-wide ${align === 'right' ? 'flex-row-reverse' : ''}`}>
          ...
          <span className={`flex flex-col text-[8px] leading-[0.55] transition-opacity ${isActive ? 'opacity-100' : 'opacity-30 group-hover:opacity-70'}`}>
            <span className={isActive && sortConfig.direction === 'asc' ? ('text-blue-400') : ''}>▲</span>
            <span className={isActive && sortConfig.direction === 'desc' ? ('text-blue-400') : ''}>▼</span>
          </span>
        </div>
      </th>
    );
    ...
    <div className="flex flex-col w-full">
      <div className="overflow-auto no-scrollbar" style={{ scrollbarWidth: 'thin' }}>
        <table className="w-full text-left text-sm whitespace-nowrap min-w-[800px]">
          <thead className={`sticky top-0 z-20 border-b ${'bg-slate-950/80 border-slate-850 backdrop-blur-sm'}`}>
            <tr>
          ...
          <tbody>
            {currentData.map((item, index, arr) => {
              ...
              const rowBg      = isAlt ? ('bg-slate-950/20') : 'bg-transparent';
              ...
              return (
                <tr key={item.id} className={`group transition-colors duration-100 border-b ${'border-slate-850/60 hover:bg-slate-850/40'} ${rowBg}`}>
                  <td className="px-4 py-2.5 align-middle">
                    {isNewDate ? (
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-black tabular-nums ${'text-slate-300'}`}>{item.date}</span>
                        <div className={`flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity`}>
                          <button onClick={() => handleOpenAddModal(item.date, 'income')} className={`p-1 rounded transition-colors ${'text-emerald-500 hover:bg-emerald-900/40'}`} title="เพิ่มรายรับวันนี้">
                            <PlusCircle className="w-3 h-3" />
                          </button>
                          <button onClick={() => handleOpenAddModal(item.date, 'expense')} className={`p-1 rounded transition-colors ${'text-red-400 hover:bg-red-900/40'}`} title="เพิ่มรายจ่ายวันนี้">
                            <PlusCircle className="w-3 h-3" />
                          </button>
                        </div>
                      </div>
                  ...
                  <td className="px-4 py-2.5 align-middle text-center">
                    <span className={`inline-flex items-center justify-center min-w-[52px] px-2 py-0.5 text-[11px] font-black rounded-sm ${isInc ? ('bg-emerald-900/40 text-emerald-400 ring-1 ring-emerald-800/60') : ('bg-red-900/30 text-red-400 ring-1 ring-red-900/50')}`}>
      ...
      <div className={`flex items-center justify-between px-4 py-3 border-t z-30 ${
        'bg-slate-950/80 border-slate-850 backdrop-blur-sm'
      }`}>
        {/* Left: Record Count */}
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 rounded-sm border transition-all text-[10px] font-black uppercase tracking-widest ${
              currentPage === 1
                ...
                : 'hover:bg-slate-850 border-slate-800 bg-slate-900 text-slate-300'
            }`}
          >
            <ChevronLeft className="w-3 h-3" /> ก่อนหน้า
          </button>
          
          <div className={`text-[11px] font-black tabular-nums ${'text-blue-400'}`}>
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end">
            <span className={`text-[9px] font-bold uppercase tracking-tighter ${'text-slate-500'}`}>รายรับในหน้านี้</span>
            <span className="text-[13px] font-black tabular-nums text-emerald-500">{formatMoney(pageInc)}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className={`text-[9px] font-bold uppercase tracking-tighter ${'text-slate-500'}`}>รายจ่ายในหน้านี้</span>
