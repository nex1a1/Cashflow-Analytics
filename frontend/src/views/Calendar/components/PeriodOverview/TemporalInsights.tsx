...
function DayOfWeekMatrixStrip({ dayOfWeekStats, peakDow }: { dayOfWeekStats: any[]; peakDow: any }) {
...
function WorkLifeFinancialsCard({ activeDayTypes, workVsRest, dayOfWeekStats, peakDow, humanInsightText }: { activeDayTypes: any[]; workVsRest: any; dayOfWeekStats: any[]; peakDow: any; humanInsightText: any }) {
...
function RankedCategoryRow({ cat }: { cat: any }) {
  const config = (ALLOCATION_CONFIG as Record<string, any>)[cat.allocation] || ALLOCATION_CONFIG.savings;
...
function AllocationRhythmCard({ allocationData }: { allocationData: any }) {
  const {
    needTotal = 0,
    wantTotal = 0,
    savingsTotal = 0,
...
const TemporalInsights = React.memo(function TemporalInsights({
  ...
  dayOfWeekStats = [],
  allocationData
}: TemporalInsightsProps): React.ReactElement {
  const workAvg = workVsRest?.workAvgExpense || 0;
  const restAvg = workVsRest?.restAvgExpense || 0;
