// src/views/Dashboard/components/ExpenseProportion.tsx
 ...
 */
const CatItem = React.memo(({ cat, idx, isHovered, onHover }: any) => (
  <div 
 ...
 */
const GroupItem = React.memo(({ item, idx, isHovered, onHover, isSingleMonthView, sortMode = 'amount-desc' }: any) => {
 ...
 */
const AllocationItem = React.memo(({ item, idx, isHovered, onHover, activeTotal = 0, excludedGroupIds = [], onToggleGroup }: any) => {
  ...
  const activeGroups = groups.filter((g: any) => !excludedGroupIds.includes(g.id));
  ...
  return (
    ...
    >
              ...
              <div className="flex flex-col w-full relative">
                ...
                <div className="w-full rounded-none h-[10px] flex gap-[1px] bg-[#181818] p-[1px] border border-[#303030]/60 relative z-30">
                  ...
                  <div className="h-full flex gap-[1px] min-w-0" style={{ width: `${withinBudgetPct}%` }}>
                    {activeGroups.map((g: any, gIdx: number) => {
      ...
      <div className="flex-1 flex flex-col min-w-0 pt-1.5 border-t border-dashed border-[#303030]/60 gap-0.5 justify-start">
        ...
        {isSavings && (() => {
          const sumGroups = activeGroups.reduce((acc: number, g: any) => acc + g.amount, 0);
...
function calculateSimulatedAllocation(isAllocationMode: boolean, excludedGroupIds: string[], sortedAllocation: any[], totalIncome: number, totalExpense: number, netCashflow: number) {
  ...
  const activeNeedsTotal = needsGroups.filter((g: any) => !excludedSet.has(g.id)).reduce((sum: number, g: any) => sum + g.amount, 0);
  const activeWantsTotal = wantsGroups.filter((g: any) => !excludedSet.has(g.id)).reduce((sum: number, g: any) => sum + g.amount, 0);
...
function sortProportionItems(rawItems: any[], sortMode: string, isAllocationMode: boolean) {
...
function buildDoughnutChartData(activeItems: any[], isGroupMode: boolean, hoveredIdx: number) {
  if (isGroupMode) {
    const outerCategories: any[] = [];
...
function resolveDoughnutHoverIndex(elements: any[], isGroupMode: boolean, activeItems: any[]) {
...
function ModeSwitcher({ displayMode, onChangeMode }: any) {
  const modes = [
...
function SortSwitcher({ sortMode, onToggleSort }: any) {
...
function SimulationBadge({ excludedCount, totalReduced, onReset }: any) {
...
function ExpenseProportionChartAnchor({ activeChartData, options, isAllocationMode, activeTotal, onMouseLeave }: any) {
  return (
    <div 
...
function ExpenseProportion() {
  ...
  const [excludedGroupIds, setExcludedGroupIds] = useState<string[]>([]); // Track excluded groups in What-If simulation mode
  ...
  const handleSortToggle = useCallback((targetType: string) => {
    setSortMode(prev => {
      ...
      {showSkeleton ? (
        ...
        <div className="flex flex-row items-stretch min-h-[140px]">
          <ExpenseProportionChartAnchor
            activeChartData={activeChartData as any}
