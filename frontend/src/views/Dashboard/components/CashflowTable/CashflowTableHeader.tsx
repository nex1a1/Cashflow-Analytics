    ...
    <thead className="sticky top-0 z-30 bg-surface">
      <tr>
        ...
        <th
          ...
          className={`px-3 py-2.5 font-bold border-l border-b ${thinBorder} align-middle sticky right-[140px] z-50 transition-colors w-[110px] min-w-[110px] max-w-[110px] ${
            hoveredCol === 'net' ? 'bg-surface-elevated text-white' : 'text-neutral-200 bg-surface'
          }`}
        >
          เงินคงเหลือ
        </th>
        ...
        <th
          ...
          className={`px-2 py-2.5 font-bold border-l border-b text-center align-middle sticky right-[70px] z-50 w-[70px] min-w-[70px] max-w-[70px] ${thinBorder} transition-colors cursor-help ${
            hoveredCol === 'pct-left' ? 'bg-surface-elevated text-white' : 'text-neutral-200 bg-surface'
          }`}
        >
          %เหลือ
        </th>
        ...
        <th
          ...
          className={`px-2 py-2.5 font-bold border-l border-r border-b text-center align-middle sticky right-0 z-50 w-[70px] min-w-[70px] max-w-[70px] ${thinBorder} transition-colors cursor-help ${
            hoveredCol === 'pct-spent' ? 'bg-surface-elevated text-white' : 'text-neutral-200 bg-surface'
          }`}
        >
          %จ่าย
        </th>
      ...
      <tr>
        {activeIncomeGroups.map((g, idx) => {
          ...
          const groupColor = g.color || tc('income');
            ...
            <React.Fragment key={g.id}>
              ...
              >
                <div className="flex items-center justify-center gap-1 min-w-0">
                  ...
                  >
                    <CategoryGlyph
                      icon={g.icon}
                      color={isExcluded ? tc('ink-muted') : groupColor}
                      size={18}
                      fallbackEmoji="💰"
                    />
                    {cats.length > 0 && (
                  ...
                  <button
                    ...
                    className={`p-1 rounded-none transition-colors inline-flex items-center justify-center shrink-0 ${
                      isExcluded
                        ? 'text-neutral-400 hover:text-neutral-300 hover:bg-surface-elevated'
                        : 'text-neutral-400 hover:text-white hover:bg-surface-elevated/50'
                    }`}
                    title={isExcluded ? `นำกลุ่ม ${g.name} กลับมารวมคำนวณ` : `ยกเว้นกลุ่ม ${g.name} จากการคำนวณ`}
                  >
                    {isExcluded ? <EyeOff className="w-3 h-3 text-ink-muted" aria-label="ยกเว้นจากการคำนวณ" /> : <Eye className="w-3 h-3" />}
                  </button>
                </div>
              </th>
              {isExpanded && cats.map((c, cIdx) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-center gap-1 min-w-0">
                      <CategoryGlyph icon={c.icon} color={isCatFaded ? tc('ink-muted') : c.color} size={15} className={isCatFaded ? 'opacity-60' : ''} fallbackEmoji="📁" />
                      <button
        ...
        {activeExpenseGroups.map((g) => {
          ...
          const isColHovered = hoveredCol === colId;
          const isExcluded = excludedGroups.has(g.id);
          const groupColor = g.color || tc('gray-300');
