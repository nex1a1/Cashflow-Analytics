...
const TransactionItem = ({ tx, index, catDef, isDarkMode, maxAmount }: TransactionItemProps) => {
...
export default function TopTransactions() {
  ...
  const { displayTransactions, maxAmount, topSum } = useMemo(() => {
    ...
    const catMap = {
      ...
      byName: new Map<string, Category>()
    };
    ...
    const getCat = (tx: TransactionDisplay) => (tx.category_id ? catMap.byId.get(tx.category_id) : undefined) || (tx.category ? catMap.byName.get(tx.category) : undefined);
    ...
    filtered = filtered.filter(tx => {
      ...
      if ((tx as any).type === 'expense') return true;
      if ((tx as any).type === 'income') return false; 
      const catDef = getCat(tx);
      return catDef ? (catDef as any).type === 'expense' : (Number(tx.amount) < 0);
    });
    ...
    if (hideFixedExpenses) {
      filtered = filtered.filter(tx => {
        const catDef = getCat(tx);
        const aType = tx.allocation_type || (catDef ? (catDef as any).allocation_type : null) || 'want';
        return aType !== 'need';
      });
    }
    ...
    const max = results.length > 0 ? Math.abs(Number(results[0].amount)) : 0;
    const sum = results.reduce((acc, tx) => acc + Math.abs(Number(tx.amount)), 0);
