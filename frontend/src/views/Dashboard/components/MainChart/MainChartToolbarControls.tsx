    ...
    <div className="flex p-[1px] bg-surface-elevated/60 gap-[1px] bg-neutral-900 rounded-none shadow-[inset_0_1px_3px_rgba(0,0,0,0.3)] shrink-0">
      <button
        ...
        className={`px-3 py-1.5 text-[11px] font-bold transition-all ${
          isAll ? 'bg-surface-elevated text-white shadow-sm' : 'bg-canvas text-slate-400 hover:text-slate-200 hover:bg-surface-elevated/50'
        }`}
...
export const ToolbarLineStyleSelector = memo(({ showSkeleton, isSmoothLine, setIsSmoothLine }: ToolbarLineStyleSelectorProps) => (
  <div className="flex p-0.5 rounded-none border shadow-sm bg-canvas border-line">
    <button
      disabled={showSkeleton}
