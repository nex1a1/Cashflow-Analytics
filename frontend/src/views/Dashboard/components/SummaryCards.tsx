...
import { 
  Activity, Wallet, Target, UtensilsCrossed,
  TrendingUp, Zap, Home, Award, TrendingDown, Navigation,
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-[1px] bg-[#2d2d2d]">
        ...
        {/* INCOME CELL */}
        <div className="group relative overflow-hidden p-3 flex flex-col justify-between min-h-[80px] border-l border-l-emerald-500 bg-[#181818] hover:bg-[#1d1d1d] transition-none">
        ...
        {/* EXPENSE CELL */}
        <div className="group relative overflow-hidden p-3 flex flex-col justify-between min-h-[80px] border-l border-l-[#da291c] bg-[#181818] hover:bg-[#1d1d1d] transition-none">
        ...
        {/* CASHFLOW CELL */}
        <div className={`group relative overflow-hidden p-3 flex flex-col justify-between min-h-[80px] border-l bg-[#181818] hover:bg-[#1d1d1d] transition-none ${
...
function getFoodIncomeStatus(pct: number) {
  ...
  return            { label: 'สัดส่วนสูง',     cls: 'text-[#da291c] border-[#da291c]/30' };
}
...
const StrategicCardShell = memo(({
  ...
  return (
    <div
      className={`group relative overflow-hidden p-3 flex flex-col justify-between h-full min-h-[148px] bg-[#181818] ${hoverBgClass} transition-none border-l ${borderColorClass} ${
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-between w-full h-full transition-none">
...
const StrategicRentCard = memo(({ rentPercentageNum, rentTotal, rentSub, showSkeleton }: RentCardProps) => {
  ...
  return (
    <StrategicCardShell
      icon={Home}
      borderColorClass={isOver ? 'border-l-[#da291c]' : 'border-l-neutral-600'}
      hoverBgClass="hover:bg-[#1c1c1c]"
      label="ภาระที่พักอาศัย (Rent Ratio)"
      thresholdRow={!showSkeleton && (
        <div className="my-auto py-1 border-y border-neutral-800/60 flex items-center justify-between text-[10px] font-bold text-neutral-400">
          <span>เกณฑ์แนะนำ</span>
          <span className="text-white font-extrabold">&lt; 30% ของรายรับ</span>
        </div>
      )}
      showOverlay={!showSkeleton}
      overlayTitle="รายละเอียดที่พัก"
      overlayBadge={<span className="text-neutral-300 font-extrabold text-[9px] border border-neutral-600/50 px-1.5 py-0.5 rounded-none leading-none whitespace-nowrap shrink-0">4 หมวด</span>}
      overlayBody={rentSub && (
      ...
      <div className="mt-auto z-10">
        {showSkeleton ? (
          ...
          <div className="flex items-baseline gap-1.5">
            <div className={`text-lg font-black ${isOver ? 'text-[#da291c]' : 'text-white'}`}>
              {rentPercentageNum.toFixed(1)}%
            </div>
            <div className="text-[10px] font-bold text-neutral-400 tabular-nums">
              ฿{formatMoney(rentTotal)}
            </div>
          </div>
        )}

        <div className="w-full h-1 mt-1.5 rounded-none bg-neutral-900 border border-neutral-800/80 overflow-hidden relative">
          <div
...
const StrategicSubscriptionCard = memo(({
  ...
  let statusBadge = {
    label: 'SAFE',       cls: 'text-neutral-300 border-neutral-600/50',
    borderLeft: 'border-l-neutral-600', barBg: 'bg-neutral-300', colorText: 'text-white'
  };
  if (isLeak) {
    statusBadge = {
  ...
  const renderSubscriptionOverlay = () => {
    ...
    if (services.length === 1) {
      return (
        <div className="col-span-2 bg-[#181818] p-2 flex items-center justify-between text-left min-w-0">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
            <CategoryGlyph icon={services[0].icon} size={13} /> {services[0].name}
          </span>
          <div className="flex items-baseline gap-1 leading-tight">
            <span className="text-[13px] font-black text-white tabular-nums">฿{formatMoney(services[0].amount)}</span>
      ...
      return (
        <>
          {services.map((item, idx) => (
            <div key={idx} className="bg-[#181818] p-2 flex flex-col justify-center text-left min-w-0">
              <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
                <CategoryGlyph icon={item.icon} size={13} /> {item.name}
              </span>
              <div className="flex items-baseline gap-1 mt-1 leading-tight">
                <span className="text-[13px] font-black text-white tabular-nums">฿{formatMoney(item.amount)}</span>
                {subscriptionTotal > 0 && (
          ...
          <div className="col-span-2 bg-[#181818] p-2 flex items-center justify-between text-left min-w-0">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
              <Repeat size={13} className="shrink-0 text-amber-400" /> ยอดรวมทั้งหมด
            </span>
            <span className="text-[13px] font-black text-white tabular-nums leading-tight">
    ...
    if (services.length === 3) {
      return (
        <>
          ...
          <div className="bg-[#181818] p-2 flex flex-col justify-center text-left min-w-0">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
              <Repeat size={13} className="shrink-0 text-amber-400" /> ยอดรวมทั้งหมด
            </span>
            <div className="flex items-baseline gap-1 mt-1 leading-tight">
              <span className="text-[13px] font-black text-white tabular-nums">฿{formatMoney(subscriptionTotal)}</span>
              <span className="text-[9px] font-bold text-neutral-400">(100%)</span>
            </div>
          </div>
        </>
...
const StrategicLifestyleCard = memo(({ lifestyleRatio, variableTotal, topWantCategories, showSkeleton }: LifestyleCardProps) => {
  ...
  const renderLifestyleOverlay = () => {
    ...
    if (categories.length === 1) {
      return (
        <div className="col-span-2 bg-[#181818] p-2 flex items-center justify-between text-left min-w-0">
          <span className="text-[10px] font-bold text-neutral-300 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
            <CategoryGlyph icon={categories[0].icon} color={categories[0].color} size={13} /> {categories[0].name}
          </span>
          <div className="flex items-baseline gap-1 leading-tight">
            <span className="text-[13px] font-black text-white tabular-nums">฿{formatMoney(categories[0].amount)}</span>
            <span className="text-[9px] font-bold text-neutral-400">({categories[0].pctOfWant}%)</span>
          </div>
        </div>
      );
    }
    ...
    if (categories.length === 2) {
      return (
        <>
          {categories.map(cat => (
            <div key={cat.id} className="bg-[#181818] p-2 flex flex-col justify-center text-left min-w-0">
              <span className="text-[10px] font-bold text-neutral-300 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
                <CategoryGlyph icon={cat.icon} color={cat.color} size={13} /> {cat.name}
              </span>
              <div className="flex items-baseline gap-1 mt-1 leading-tight">
                <span className="text-[13px] font-black text-white tabular-nums">฿{formatMoney(cat.amount)}</span>
                <span className="text-[9px] font-bold text-neutral-400">({cat.pctOfWant}%)</span>
              </div>
            </div>
          ))}
          <div className="col-span-2 bg-[#181818] p-2 flex items-center justify-between text-left min-w-0">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
              <Zap size={13} className="shrink-0 text-neutral-400" /> รวมฟุ่มเฟือยทั้งหมด
            </span>
            <span className="text-[13px] font-black text-white tabular-nums leading-tight">
    ...
    if (categories.length === 3) {
      return (
        <>
          ...
          <div className="bg-[#181818] p-2 flex flex-col justify-center text-left min-w-0">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide truncate flex items-center gap-1.5 leading-none">
              <Zap size={13} className="shrink-0 text-neutral-400" /> รวมฟุ่มเฟือย
            </span>
            <div className="flex items-baseline gap-1 mt-1 leading-tight">
              <span className="text-[13px] font-black text-white tabular-nums">฿{formatMoney(variableTotal)}</span>
              <span className="text-[9px] font-bold text-neutral-400">(100%)</span>
            </div>
          </div>
        </>
  ...
  return (
    <StrategicCardShell
      ...
      overlayBadge={<span className="text-neutral-300 font-extrabold text-[9px] border border-neutral-600/50 px-1.5 py-0.5 rounded-none leading-none whitespace-nowrap shrink-0">Top 4</span>}
      overlayBody={(
        <div className="grid grid-cols-2 gap-[1px] bg-neutral-800/50 mt-1.5 flex-1">
          {renderLifestyleOverlay()}
        </div>
      )}
    >
      <div className="mt-auto z-10">
        ...
        <div className="w-full h-1 mt-1.5 rounded-none bg-neutral-900 border border-neutral-800/80 overflow-hidden relative">
          <div
            className={`h-full absolute left-0 top-0 transition-none ${showSkeleton ? 'bg-slate-700 animate-pulse' : isOver ? 'bg-[#da291c]' : 'bg-neutral-300'}`}
    ...
    <StrategicCardShell
      ...
      thresholdRow={!showSkeleton && (
        <div className="my-auto py-1 border-y border-neutral-800/60 flex items-center justify-between text-[10px] font-bold text-neutral-400">
          ...
          <span className="text-white font-extrabold">{periodDays} วันในงวด</span>
        </div>
      )}
      showOverlay={!showSkeleton}
    ...
    >
      <div className="mt-auto z-10">
        {showSkeleton ? (
          ...
          <div className={`text-lg font-black tabular-nums leading-none ${isPositive ? 'text-emerald-400' : 'text-[#da291c]'}`}>
            {formatSignedMoney(dailyVictory)}
          </div>
        )}
        <div className="text-[10px] font-bold text-neutral-400 mt-1 leading-none">
    ...
    <StrategicCardShell
      ...
      thresholdRow={!showSkeleton && (
        <div className="my-auto py-1 border-y border-neutral-800/60 flex items-center justify-between text-[10px] font-bold text-neutral-400">
          ...
          <span className="text-white font-extrabold">&lt; 25% ของรายจ่าย</span>
        </div>
      )}
      showOverlay={!showSkeleton}
      overlayTitle="เจาะลึกงบอาหาร"
      overlayBadge={<span className={`font-extrabold text-[9px] border px-1.5 py-0.5 rounded-none leading-none whitespace-nowrap shrink-0 ${foodStatus.cls}`}>{foodStatus.label}</span>}
      overlayBody={(
        <div className="grid grid-cols-2 gap-[1px] bg-neutral-800/50 mt-1.5 flex-1">
          <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
              <UtensilsCrossed size={13} className="shrink-0 text-neutral-400" /> รวมค่าอาหาร
            </span>
            <span className="text-[13px] font-black text-white tabular-nums leading-tight mt-1">฿{formatMoney(foodTotal)}</span>
          </div>
          <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
              <TrendingDown size={13} className="shrink-0 text-neutral-400" /> % รายจ่าย
            </span>
            <span className="text-[13px] font-black text-white tabular-nums leading-tight mt-1">{foodPercentage}%</span>
          </div>
          {/* vs Expense threshold */}
          <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide leading-none">เทียบเกณฑ์ 25%</span>
            <span className={`text-[11px] font-black tabular-nums leading-tight mt-1 ${pctOfExpense > 25 ? 'text-[#da291c]' : 'text-emerald-400'}`}>
              {vsExpThreshold}
            </span>
          </div>
          {/* vs Income threshold */}
          <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide leading-none">เทียบรายรับ 20%</span>
            <span className={`text-[11px] font-black tabular-nums leading-tight mt-1 ${pctOfIncome > 20 ? 'text-[#da291c]' : 'text-emerald-400'}`}>
              {vsIncThreshold}
            </span>
          </div>
        </div>
    ...
    >
      <div className="mt-auto z-10">
        {showSkeleton ? (
          ...
          <div className={`text-lg font-black leading-none ${isOver ? 'text-[#da291c]' : 'text-white'}`}>
            {foodPercentage}%
          </div>
        )}
        <div className="text-[10px] font-bold text-neutral-400 mt-1 leading-none">
...
const StrategicFoodDailyCard = memo(({ foodDailyAvg, foodTotal, foodWorkdayAvg, foodHolidayAvg, maxFoodDayAmount, showSkeleton }: FoodDailyCardProps) => (
  <StrategicCardShell
    icon={UtensilsCrossed}
    borderColorClass="border-l-neutral-600"
    label="กินเฉลี่ย/วัน"
    thresholdRow={!showSkeleton && (
      <div className="my-auto py-1 border-y border-neutral-800/60 flex items-center justify-between text-[10px] font-bold text-neutral-400">
        <span>งบกินรวม</span>
        <span className="text-white font-extrabold tabular-nums">฿{formatMoney(foodTotal)}</span>
      </div>
    )}
    showOverlay={!showSkeleton}
    overlayTitle="พฤติกรรมการกิน"
    overlayBadge={<span className="text-neutral-300 font-extrabold text-[9px] border border-neutral-600/50 px-1.5 py-0.5 rounded-none leading-none whitespace-nowrap shrink-0">ทำงาน vs หยุด</span>}
    overlayBody={(
      <div className="grid grid-cols-2 gap-[1px] bg-neutral-800/50 mt-1.5 flex-1">
        <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
            <Briefcase size={13} className="shrink-0 text-neutral-400" /> วันทำงาน
          </span>
          <span className="text-[13px] font-black text-white tabular-nums leading-tight mt-1">฿{formatMoney(foodWorkdayAvg)}</span>
        </div>
        <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
            <Palmtree size={13} className="shrink-0 text-neutral-400" /> วันหยุด
          </span>
          <span className="text-[13px] font-black text-white tabular-nums leading-tight mt-1">฿{formatMoney(foodHolidayAvg)}</span>
        </div>
        <div className="col-span-2 bg-[#181818] p-2 flex flex-col justify-center text-left">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
            <Trophy size={13} className="shrink-0 text-neutral-400" /> พีคสูงสุดใน 1 วัน
          </span>
          <span className="text-[13px] font-black text-white tabular-nums leading-tight mt-1">฿{formatMoney(maxFoodDayAmount)}</span>
        </div>
      </div>
    )}
  >
  ...
  <StrategicCardShell
    ...
    thresholdRow={!showSkeleton && (
      <div className="my-auto py-1 border-y border-neutral-800/60 flex items-center justify-between text-[10px] font-bold text-neutral-400">
        ...
        <span className="text-white font-extrabold tabular-nums">฿{formatMoney(totalExpense)}</span>
      </div>
    )}
    showOverlay={!showSkeleton}
    ...
    overlayBadge={<span className="text-neutral-300 font-extrabold text-[9px] border border-neutral-600/50 px-1.5 py-0.5 rounded-none leading-none whitespace-nowrap shrink-0">ทำงาน vs หยุด</span>}
    overlayBody={(
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-[1px] bg-neutral-800/50 mt-1.5 flex-1">
        <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
            <Briefcase size={13} className="shrink-0 text-neutral-400" /> วันทำงาน
          </span>
          <span className="text-[13px] font-black text-white tabular-nums leading-tight mt-1">฿{formatMoney(dailyWorkdayAvg)}</span>
        </div>
        <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
            <Palmtree size={13} className="shrink-0 text-neutral-400" /> วันหยุด
          </span>
          <span className="text-[13px] font-black text-white tabular-nums leading-tight mt-1">฿{formatMoney(dailyHolidayAvg)}</span>
        </div>
        <div className="bg-[#181818] p-2 flex flex-col justify-center text-left">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wide flex items-center gap-1.5 leading-none">
            <Lock size={13} className="shrink-0 text-neutral-400" /> จำเป็น/วัน
...
const SummaryStrategic = memo(({ analytics, showSkeleton }: SummaryStrategicProps) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-[1px] bg-[#2d2d2d]">
        ...
        {/* Spans the remaining slot so a 7-card grid never leaves a dangling empty cell */}
...
const SummaryForecasting = memo(({ analytics, showSkeleton }: SummaryForecastingProps) => {
  ...
  return (
    <div className="flex flex-col">
      {/* Header */}
      <div className="px-4 py-2 flex items-center justify-between border-b border-[#2d2d2d] bg-[#121212]/90">
        <div className="flex items-center gap-2">
      ...
      <div className="px-4 py-2 bg-[#151515] border-b border-[#2d2d2d] flex flex-col gap-1.5 text-xs">
        {/* Row A: pace label + badge + ceiling */}
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <Gauge className="w-4 h-4 text-neutral-300 shrink-0" />
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-neutral-400 font-mono">
            ...
            <span className="text-neutral-400"> ({monthProgressPct}%)</span>
          </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[1px] bg-[#2d2d2d]">
        ...
        {/* Card 1: Projected Expense (plain — Safe-to-Spend carries the state-driven weight) */}
        <div className="relative p-4 flex flex-col gap-2.5 border-l border-l-neutral-800/60">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase tracking-wider text-neutral-300">พยากรณ์รายจ่ายเดือนนี้</span>
              <span className="text-[10px] font-mono font-bold text-neutral-500">MONTHLY FORECAST</span>
            </div>
            {showSkeleton ? (
              <Shimmer className="h-8 w-32 mt-2" />
            ) : (
              <div>
                <div className="text-2xl font-black text-white tabular-nums tracking-tight leading-none mt-1.5">
          ...
          <div className="space-y-1">
            <div className="h-1.5 w-full bg-neutral-800 flex overflow-hidden border border-neutral-800">
              <div style={{ width: `${fixedPct}%` }}   className="bg-slate-400" title="Fixed" />
              <div style={{ width: `${varSpentPct}%` }} className="bg-neutral-400" title="Var Spent" />
              <div style={{ width: `${varRemPct}%` }}   className="bg-neutral-700/80" title="Var Projected" />
            </div>
            <div className="flex justify-between text-[10px] font-mono text-neutral-400">
              <span className="text-slate-300">ภาระคงที่ ({fixedPct.toFixed(0)}%)</span>
              <span className="text-neutral-300">จ่ายแล้ว ({varSpentPct.toFixed(0)}%)</span>
              <span className="text-neutral-400">ประเมิน ({varRemPct.toFixed(0)}%)</span>
            </div>
          </div>

          <div className="pt-2 border-t border-neutral-800 text-xs font-mono text-neutral-300 space-y-1">
        ...
        <div className={`group relative overflow-hidden p-4 flex flex-col gap-2.5 transition-none border-l ${
          safeToSpend > 300
          ...
          <div>
            ...
            {showSkeleton ? (
              ...
              <div>
                <div className={`text-2xl font-black tabular-nums tracking-tight leading-none mt-1.5 ${
          ...
          <div className="pt-2 border-t border-neutral-800/80 text-xs font-mono text-neutral-300 space-y-1">
            <div className="flex justify-between items-center">
          ...
          <div className={`absolute -right-3 -bottom-3 opacity-[0.03] pointer-events-none ${
            safeToSpend > 300 ? 'text-emerald-500' : 'text-[#da291c]'
          }`}>
            <Zap size={80} />
          </div>
        </div>

        {/* Card 3: Projected Net Cashflow (plain — Safe-to-Spend carries the state-driven weight) */}
        <div className="relative p-4 flex flex-col gap-2.5 border-l border-l-neutral-800/60">
          <div>
            <div className="flex items-center justify-between">
          ...
          <div className="space-y-1">
            <div className="h-1.5 w-full bg-neutral-800 overflow-hidden relative border border-neutral-800">
              <div
                className={`h-full ${projectedSurplus >= 0 ? 'bg-emerald-500' : 'bg-[#da291c]'}`}
                style={{ width: `${Math.min(100, Math.max(0, projectedSurplusPct))}%` }}
              />
            </div>
            <div className="flex justify-between text-[10px] font-mono text-neutral-400">
              <span>เงินสดคงเหลือคาดการณ์:</span>
              <span className="text-neutral-300 font-bold">{projectedSurplusPct}% ของรายรับ</span>
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-neutral-800 text-xs font-mono text-neutral-300 space-y-1">
            <div className="flex justify-between items-center">
              ...
              <span className="text-neutral-300 font-bold">฿{formatMoney(maxAllowedExpense)}</span>
            </div>
            {requiredReduction > 0 ? (
              <div className="flex justify-between items-center text-[#da291c] font-black border-t border-neutral-800 pt-1">
                <span>• ต้องคุมรายจ่ายลงอีกรวม:</span>
                <span>-฿{formatMoney(requiredReduction)}</span>
              </div>
            ) : (
              <div className="flex justify-between items-center text-emerald-400 font-bold border-t border-neutral-800 pt-1">
