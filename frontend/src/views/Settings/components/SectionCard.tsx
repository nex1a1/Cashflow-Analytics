...
const SectionCard = memo(function SectionCard({
  ...
  return (
    <div className={`overflow-hidden rounded-none border bg-[#1c1c1c] border-[#303030] ${a.glow[mode]}`}>
      <div className={`px-3.5 py-2 flex items-center justify-between gap-3 border-b border-[#303030]/60 ${a.header[mode]}`}>
        <h2 className={`text-[11px] font-black uppercase tracking-widest flex items-center gap-1.5 ${a.title[mode]}`}>
          {icon}
          {title}
          {badge != null && (
            <span className="text-[9px] font-mono font-black px-1.5 py-0.5 ml-1 rounded-none tabular-nums bg-[#121212] text-[#888888] border border-[#303030]">
              {badge}
            </span>
          )}
        </h2>
