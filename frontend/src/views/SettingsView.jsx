  ...
  return (
    <button onClick={handleClick} disabled={disabled}
      className={`rounded-sm font-bold transition-all active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed ${
        confirming ? 'bg-red-500 text-white px-3 py-1.5 text-xs animate-pulse' : size === 'lg'
            ? `px-5 py-2.5 flex items-center gap-2 text-sm ${isDarkMode ? 'bg-red-900/40 text-red-100 hover:bg-red-600 hover:text-white border border-red-800' : 'bg-red-50 text-red-600 hover:bg-red-600 hover:text-white border border-red-200'}`
            : `p-1 ${isDarkMode ? 'text-slate-500 hover:text-white hover:bg-red-500/80' : 'text-slate-400 hover:text-white hover:bg-red-500'}`
      }`} title={disabled ? tooltip : (confirming ? 'ยืนยันการลบ?' : tooltip)}>
      {confirming ? 'ยืนยันการลบ?' : size === 'lg' ? <><Trash2 className="w-4 h-4" /> ล้างข้อมูลทั้งหมด</> : <Trash2 className="w-4 h-4" />}
    </button>
  );
}

/* ── Color Picker & AutoFocusInput components remain the same... ── */
function ColorPicker({ color, onChange, isDarkMode }) {
  ...
  const COLOR_PALETTE = ['#FF0000','#EF4444','#DC2626','#B91C1C','#7F1D1D','#F97316','#EA580C','#C2410C','#F43F5E','#E11D48','#FB923C','#FBBF24','#F59E0B','#D97706','#B45309','#EAB308','#CA8A04','#A3E635','#84CC16','#65A30D','#22C55E','#16A34A','#15803D','#166534','#10B981','#059669','#047857','#14B8A6','#0D9488','#0F766E','#06B6D4','#0891B2','#0E7490','#22D3EE','#67E8F9','#3B82F6','#2563EB','#1D4ED8','#1E40AF','#0EA5E9','#6366F1','#4F46E5','#4338CA','#3730A3','#818CF8','#8B5CF6','#7C3AED','#A855F7','#9333EA','#EC4899'];
  return (
  ...
  return (
    <div className={`flex flex-nowrap items-center gap-2 px-3 py-1.5 border-b last:border-0 transition-colors group/cat ${!isIncome && cat.isFixed ? (dm ? 'bg-purple-900/10 hover:bg-purple-900/20' : 'bg-purple-50/30 hover:bg-purple-50/60') : (dm ? 'hover:bg-slate-800/60' : 'hover:bg-slate-50')} ${dm ? 'border-slate-700/60' : 'border-slate-100'}`}>
      <div className={`flex flex-col items-center shrink-0 opacity-0 group-hover/cat:opacity-100 transition-opacity ${dm ? 'text-slate-500' : 'text-slate-400'}`}>
        ...
        <button type="button" onClick={() => onMove(cat.id, 'DOWN')} className={`p-0.5 rounded-sm mt-0.5 ${dm ? 'hover:text-slate-200 hover:bg-slate-700' : 'hover:text-slate-800 hover:bg-slate-200'}`}><ChevronDown className="w-3 h-3" /></button>
      </div>
...
export default function SettingsView({ 
  ...
  const onAddCategory = (type) => { handleAddCategory(type); setTimeout(() => { const added = [...categories].reverse().find(c => c.type === type); if (added) setNewCatId(added.id); }, 0); };
  const handleAddCashflowGroup = () => { const newGroup = { id: `cg_${Date.now()}`, name: 'คอลัมน์ใหม่', type: 'expense', isDefault: false, order: cashflowGroups.length + 1, color: '#6366F1', highlightBg: false }; const updated = [...cashflowGroups, newGroup]; setCashflowGroups(updated); saveSettingToDb(CASHFLOW_GROUPS_KEY, updated); };
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full pb-10 max-w-[1400px] mx-auto px-4 pt-4">
      ...
      <div className="mb-5">
        ...
        <div className={`rounded-sm px-4 py-3 border flex items-start gap-2 shadow-sm ${dm ? 'bg-blue-900/20 border-blue-800/50' : 'bg-blue-50 border-blue-200'}`}>
          ...
          <div className={`text-xs leading-relaxed ${dm ? 'text-blue-200' : 'text-blue-800'}`}>
            <span className="font-bold">Fixed:</span> ใช้ตั้งค่ารายจ่ายคงที่ &nbsp;·&nbsp;
            <span className="font-bold">คอลัมน์ตาราง:</span> ตั้งชื่อกลุ่มและเลือกเปิด Bg เพื่อเทสีพื้นหลังให้คอลัมน์ในหน้า Dashboard
          </div>
        </div>
      </div>

      {/* ── Main Configurations Grid ── */}
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_480px] gap-5 items-start mb-8">
        {/* Categories column */}
        <div className="flex flex-col gap-5">
          <div className={card}>
            ...
            <div>{categories.filter(c => c.type === 'income').map(cat => (<CategoryRow key={cat.id} cat={cat} isNew={cat.id === newCatId} isIncome={true} isDarkMode={dm} onMove={handleMoveCategory} onChange={handleCategoryChange} onDelete={handleDeleteCategory} cashflowGroups={cashflowGroups} />))}</div>
          </div>
          <div className={card}>
            ...
            <div>{categories.filter(c => c.type === 'expense').map(cat => (<CategoryRow key={cat.id} cat={cat} isNew={cat.id === newCatId} isIncome={false} isDarkMode={dm} onMove={handleMoveCategory} onChange={handleCategoryChange} onDelete={handleDeleteCategory} cashflowGroups={cashflowGroups} />))}</div>
          </div>
        </div>
              ...
              {cashflowGroups.sort((a,b) => a.order - b.order).map(group => (
                <div key={group.id} className={`flex items-center gap-1.5 p-2 border rounded-sm transition-colors group/cg ${dm ? 'bg-slate-800/80 border-slate-700 hover:bg-slate-800' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'}`}>
                  <div className={`flex flex-col items-center shrink-0 opacity-0 group-hover/cg:opacity-100 transition-opacity ${dm ? 'text-slate-500' : 'text-slate-400'}`}>
                    ...
                    <button type="button" onClick={() => handleMoveCashflowGroup(group.id, 'DOWN')} className={`p-0.5 rounded-sm mt-0.5 ${dm ? 'hover:text-purple-400 hover:bg-slate-700' : 'hover:text-purple-600 hover:bg-slate-200'}`}><ChevronDown className="w-3 h-3" /></button>
                  </div>
                  ...
                  <label className={`flex items-center gap-1 cursor-pointer px-1 py-0.5 border rounded-sm text-[10px] font-bold shrink-0 transition-colors ${group.highlightBg ? (dm ? 'bg-amber-900/40 text-amber-400 border-amber-800/50' : 'bg-amber-100 text-amber-700 border-amber-300') : (dm ? 'text-slate-500 border-slate-700 hover:border-slate-500' : 'text-slate-400 border-slate-300 hover:border-slate-400')}`} title="เทสีพื้นหลังคอลัมน์">
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full ${group.highlightBg ? 'bg-amber-500' : (dm?'bg-slate-600':'bg-slate-300')}`}></span> Bg
                  </label>
      ...
      <div className={`rounded-md border-2 overflow-hidden shadow-lg ${dm ? 'bg-red-950/20 border-red-900/50' : 'bg-red-50 border-red-200'}`}>
        <div className={`px-5 py-3 border-b-2 flex items-center gap-2 ${dm ? 'bg-red-900/30 border-red-900/50' : 'bg-red-100 border-red-200'}`}>
          <AlertCircle className={`w-5 h-5 ${dm ? 'text-red-400' : 'text-red-600'}`} />
          <h2 className={`text-base font-black ${dm ? 'text-red-400' : 'text-red-700'}`}>Danger Zone: พื้นที่อันตราย</h2>
        </div>
        
        <div className={`px-6 py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 ${dm ? 'bg-slate-900/40' : 'bg-white/80'}`}>
          <div className="flex-1">
            <h3 className={`text-sm font-black mb-1 ${dm ? 'text-slate-200' : 'text-slate-800'}`}>ล้างข้อมูลทั้งหมด (Factory Reset)</h3>
            <p className={`text-xs leading-relaxed opacity-80 ${dm ? 'text-slate-400' : 'text-slate-600'}`}>
              การดำเนินการนี้จะ <strong className="underline text-red-500">ลบรายการบัญชีทั้งหมด</strong>, <strong className="underline text-red-500">ประวัติปฏิทิน</strong> และ <strong className="underline text-red-500">รีเซ็ตการตั้งค่า</strong> กลับเป็นค่าเริ่มต้นจากโรงงาน 
              <br className="hidden sm:block" /> ⚠️ ข้อมูลที่ถูกลบไปแล้ว <span className="font-black text-red-600">จะไม่สามารถกู้คืนกลับมาได้อีก</span> โปรดพิจารณาอย่างรอบคอบก่อนกดปุ่ม
            </p>
          </div>
          
          <div className="shrink-0">
            <ConfirmDeleteButton 
              onConfirm={() => handleDeleteAllData({ setShowToast: () => {} })} 
              isDarkMode={dm} 
              size="lg" 
            />
          </div>
