...
const WIDE_HEADER_COLORS: Record<string, string> = {
  'Date': 'text-[#888888]',
...
function getHeaderColorClass(header: string, format: string) {
  if (format === 'long') return LONG_HEADER_COLORS[header] || 'text-[#888888]';
  return WIDE_HEADER_COLORS[header] || 'text-[#888888]';
}

function downloadSampleFile(filename: string, content: string) {
...
function GuideSpecsPanel({ selectedFormat }: { selectedFormat: string }) {
...
function GuideFormatChooser({ selectedFormat, onSelectFormat }: { selectedFormat: string; onSelectFormat: (fmt: string) => void }) {
  const isLong = selectedFormat === 'long';
...
function GuideTemplateTools({ copied, onCopy, onDownload }: { copied: boolean; onCopy: () => void; onDownload: () => void }) {
  return (
    <section className="space-y-2.5">
      <h4 className="text-[11px] font-bold uppercase tracking-widest text-[#cbd5e1] block">
...
function GuideFormatSummary({ selectedFormat }: { selectedFormat: string }) {
  const isLong = selectedFormat === 'long';
  return (
...
function GuidePreviewSheet({ selectedFormat }: { selectedFormat: string }) {
  const isLong = selectedFormat === 'long';
...
function GuideFooter({ selectedFormat, onClose }: { selectedFormat: string; onClose: () => void }) {
  const isLong = selectedFormat === 'long';
