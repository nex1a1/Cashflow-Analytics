...
export interface ExportParamsPanelProps {
  exportPeriod?: any;
  setExportPeriod?: any;
  groupedOptions?: any;
  exportFormat?: any;
...
export interface ExportLongPreviewProps {
  dataToExport?: any;
  categories?: any;
  getDayTypeInfo?: any;
}

export interface ExportWidePreviewProps {
  dataToExport?: any;
  categories?: any;
  getDayTypeInfo?: any;
}

export interface ExportFullPreviewProps {
...
function calculateCategoryTotal(transactions: any[], date: string, categoryName: string) {
  return transactions
    .filter((t: any) => t.date === date && t.category === categoryName)
    .reduce((sum: number, t: any) => sum + Number.parseFloat(t.amount || 0), 0);
}

function buildLongCsv(dataToExport: any[], categories: any[], getDayTypeInfo: (d: string) => any, dlm: string) {
  const headers = ['Date', 'DayType', 'Type', 'Category', 'Description', 'Amount', 'Note'];
  let content = headers.join(dlm) + '\n';
  dataToExport.forEach((t: any) => {
    const cat = categories.find((c: any) => c.name === t.category);
...
function buildWideCsv(dataToExport: any[], categories: any[], getDayTypeInfo: (d: string) => any, dlm: string) {
  const dates = Array.from(new Set(dataToExport.map((t: any) => t.date as string))).sort((a: string, b: string) => a.localeCompare(b));
...
function buildFullCsv(localTransactions: any[], categories: any[], dayTypes: any, dayTypeConfig: any[], dlm: string) {
...
function filterExportTransactions(localTransactions: any[], exportPeriod: string, typeFilter: string, previewSearch: string, categories: any[]) {
  const query = previewSearch.trim().toLowerCase();
  return localTransactions.filter((t: any) => {
...
function resolveDayTypeInfo(date: string, dayTypes: any, dayTypeConfig: any[]) {
  const dtId = dayTypes[date];
  if (dtId) {
    const config = dayTypeConfig.find((d: any) => d.id === dtId);
    return {
...
function calculateExportStats(exportFormat: string, dataToExport: any[], localTransactions: any[], categories: any[], dayTypeConfig: any[], dayTypes: any) {
  ...
  if (exportFormat === 'full') {
    ...
    rowCount = new Set(dataToExport.map((t: any) => t.date)).size;
  }
...
interface CompileCsvDataProps {
  exportFormat: string;
  dataToExport: any[];
  categories: any[];
  getDayTypeInfo: (d: string) => any;
  delimiter: string;
  localTransactions: any[];
  dayTypes: any;
  dayTypeConfig: any[];
...
function compileCsvData({ exportFormat, dataToExport, categories, getDayTypeInfo, delimiter, localTransactions, dayTypes, dayTypeConfig }: CompileCsvDataProps) {
  let csvContent = '\uFEFF'; // BOM
...
const ExportHeader = ({  onClose, isExporting  }: ExportHeaderProps) => (
...
const ExportFormatOption = ({  formatKey, activeFormat, onClick, icon: Icon, title, description  }: ExportFormatOptionProps) => {
  ...
  return (
    ...
    >
      <Icon className={`w-4.5 h-4.5 shrink-0 ${isActive ? 'text-white' : 'text-[#555555]'}`} />
      <div className="flex-1">
        <h5 className="text-xs font-bold uppercase tracking-wide">{title}</h5>
        <p className="text-[10px] text-[#888888] mt-1 leading-tight">{description}</p>
      </div>
      {isActive && <div className="w-1.5 h-1.5 rounded-none bg-[#da291c] shrink-0" />}
    </button>
  );
};

const ExportParamsPanel = ({ 
  exportPeriod,
  setExportPeriod,
  groupedOptions,
  exportFormat,
    ...
    <section className="space-y-2.5">
      ...
      <div className="space-y-2.5">
        <ExportFormatOption
          ...
          icon={ClipboardList}
          title="รายงานแยกประเภทรายการ (Long)"
          description="ตารางรายการเรียงตามลำดับวันที่ เหมาะสำหรับการเก็บประวัติ"
        />
        <ExportFormatOption
          formatKey="wide"
          activeFormat={exportFormat}
          ...
          title="ตารางสเปรดชีตวิเคราะห์ (Wide Matrix)"
          description="ตารางเปรียบเทียบหมวดหมู่รายวัน เหมาะสำหรับรัน Pivot Excel"
        />
        <ExportFormatOption
          formatKey="full"
          activeFormat={exportFormat}
          onClick={setExportFormat}
          icon={Database}
          title="สำเนาฐานข้อมูลของระบบ (System Backup)"
          description="เก็บสำเนาค่าระบบ ปฏิทินวันทำงาน และหมวดหมู่ทั้งหมด"
        />
      </div>
    </section>
...
const ExportLongPreview = ({  dataToExport, categories, getDayTypeInfo  }: ExportLongPreviewProps) => (
  <table className="w-full text-left font-mono text-[11px] leading-relaxed border-collapse">
    ...
    <tbody className="divide-y divide-[#303030]">
      {dataToExport.slice(0, 15).map((t: any, idx: number) => {
        const cat = categories.find((c: any) => c.name === t.category);
        const dt = getDayTypeInfo(t.date);
...
const ExportWidePreview = ({  dataToExport, categories, getDayTypeInfo  }: ExportWidePreviewProps) => {
  const dates: string[] = (Array.from(new Set(dataToExport.map((t: any) => String(t.date)))) as string[]).sort((a, b) => a.localeCompare(b));
  const cats = categories.filter((c: any) => dataToExport.some((t: any) => t.category === c.name));
...
const ExportFullPreview = ({  localTransactions, categories, dayTypes, dayTypeConfig  }: ExportFullPreviewProps) => (
...
const ExportFooter = ({  delimiter, exportPeriod, getFilterLabel, onClose, executeExport, stats, exportFormat, isExporting  }: ExportFooterProps) => (
...
export default function ExportModal({
  ...
  const getDayTypeInfo = (date: string) => resolveDayTypeInfo(date, dayTypes, dayTypeConfig);
