...
export default function ImportPreviewModal({ importPreview, setImportPreview, confirmImport, isProcessing, categories }) {
  ...
  const inputCls = `w-full bg-transparent outline-none text-xs px-1.5 py-1 rounded-sm border focus:ring-1 transition-colors ${'border-slate-600 text-slate-200 focus:border-blue-500 hover:bg-slate-800'}`;
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 z-[100] flex items-center justify-center backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className={`rounded-sm shadow-2xl flex flex-col w-full max-w-3xl animate-in zoom-in-95 duration-200 border ${'bg-slate-900 border-slate-700'}`}
        style={{ maxHeight: 'calc(100vh - 48px)' }}>

        {/* Header */}
        <div className={`px-5 py-4 border-b flex justify-between items-center shrink-0 ${'border-slate-700'}`}>
          <div>
            <h3 className={`text-base font-bold ${'text-slate-100'}`}>📋 ตรวจสอบก่อน Import</h3>
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              <span className={`text-xs ${'text-slate-400'}`}>
                พบ <strong className={'text-blue-400'}>{allItems.length} รายการ</strong>
              </span>
              ...
              <span className={`text-[10px] ${'text-slate-500'}`}>แก้ไขได้ก่อน import</span>
            </div>
          </div>
          <button onClick={() => setImportPreview(null)} className={`p-1.5 rounded-sm ${'text-slate-400 hover:bg-slate-800'}`}>
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Table header */}
        <div className={`grid grid-cols-[55px_120px_1fr_80px_36px] gap-2 px-4 py-2 text-[10px] font-bold border-b shrink-0 ${'bg-slate-800/60 border-slate-700 text-slate-400'}`}>
        ...
        <div className="flex-1 overflow-y-auto min-h-0" style={{ scrollbarWidth: 'thin' }}>
          {pageItems.length === 0 ? (
            <div className={`p-8 text-center text-sm ${'text-slate-500'}`}>ไม่มีรายการข้อมูล</div>
          ) : (
            pageItems.map((item, idx) => {
              ...
              return (
                <div key={item.id}>
                  {isNewDate && (
                    <div className={`px-4 py-1.5 text-[10px] font-black sticky top-0 z-10 border-b ${'bg-slate-800 text-slate-300 border-slate-700'}`}>
                      {item.date}
                    </div>
                  )}
                  <div className={`grid grid-cols-[55px_120px_1fr_80px_36px] gap-2 px-4 py-1.5 items-center border-b transition-colors ${'border-slate-800 hover:bg-slate-800/40'}`}>
                    <span className={`text-[9px] font-bold px-1 py-0.5 rounded-sm text-center truncate ${isInc ? ('bg-emerald-900/40 text-emerald-400') : ('bg-red-900/40 text-red-400')}`}>
                      {isInc ? 'รายรับ' : 'รายจ่าย'}
                    </span>
                    <select value={item.category} onChange={e => updateItem(item.id, 'category', e.target.value)}
                      className={`text-[10px] font-bold py-1 px-1 rounded-sm border outline-none cursor-pointer w-full ${'bg-slate-800 border-slate-600 text-slate-300'}`}>
                    ...
                    <button onClick={() => deleteItem(item.id)} className={`p-1 rounded-sm transition-colors ${'text-slate-600 hover:text-red-400 hover:bg-red-900/30'}`}>
        ...
        {/* Footer */}
        <div className={`px-5 py-3 border-t flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0 ${'border-slate-700 bg-slate-800/50'}`}>
          <div className="flex items-center gap-2">
            <button onClick={() => setPreviewPage(p => Math.max(1, p - 1))} disabled={previewPage === 1}
              className={`p-1.5 rounded-sm border disabled:opacity-40 transition-all active:scale-95 ${'border-slate-600 text-slate-300 hover:bg-slate-700'}`}><ChevronLeft className="w-4 h-4" /></button>
            <span className={`text-xs font-medium ${'text-slate-400'}`}>หน้า {previewPage}/{totalPages} ({allItems.length} รายการ)</span>
            <button onClick={() => setPreviewPage(p => Math.min(totalPages, p + 1))} disabled={previewPage === totalPages || totalPages === 0}
              className={`p-1.5 rounded-sm border disabled:opacity-40 transition-all active:scale-95 ${'border-slate-600 text-slate-300 hover:bg-slate-700'}`}><ChevronRight className="w-4 h-4" /></button>
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <button onClick={() => setImportPreview(null)}
              className={`flex-1 sm:flex-none px-4 py-2 rounded-sm font-bold text-xs border transition-all active:scale-95 ${'border-slate-600 text-slate-300 hover:bg-slate-700'}`}>
              ยกเลิก
            </button>
