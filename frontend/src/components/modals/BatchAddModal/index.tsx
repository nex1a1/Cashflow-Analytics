...
import { Category, CashflowGroup, DayType, FrequentItem, TransactionDisplay, AllocationType } from '../../../types';
...
export interface BatchAddModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaveBatch: (items: any[]) => Promise<any> | void;
  categories: Category[];
...
export default function BatchAddModal({
  ...
  const handleAddSubmit = useCallback((data: any) => {
  ...
  const handleRemovePending = useCallback((id: string) => setPendingItems(prev => prev.filter(i => i.id !== id)), []);
  ...
  const submitBatch = async () => {
    ...
    try {
      ...
      console.error(err);
      showToast('⚠️ เกิดข้อผิดพลาดในการบันทึกข้อมูล: ' + (err?.message || String(err)), 'error');
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-3 sm:p-6">
      <div 
        className={`rounded-none shadow-2xl shadow-black/80 flex flex-col w-full max-w-[1460px] h-[85vh] min-h-[520px] max-h-[850px] overflow-hidden border ${tokens.surface}`}
