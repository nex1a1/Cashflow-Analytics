...
const recreateTriggersAndIndexes = () => {
    try {
        db_1.default.exec(`
 ...
 */
const migrateTablesToStrict = () => {
    const tables = ['settings', 'transactions', 'calendar_days', 'categories', 'day_types', 'cashflow_groups'];
    const tablesToMigrate = [];
    for (const table of tables) {
        const row = db_1.default.prepare("SELECT sql FROM sqlite_schema WHERE type = 'table' AND name = ?").get(table);
    ...
    const migration = db_1.default.transaction(() => {
...
const initSchema = () => {
  ...
  `);
    ...
    try {
        db_1.default.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)")
...
const verifyTransactionColumns = () => {
    ...
    if (!txCols.has('category_id')) {
        try {
            db_1.default.exec("ALTER TABLE transactions ADD COLUMN category_id TEXT DEFAULT '1'");
...
const verifyCategoryColumns = () => {
    ...
    if (catCols.length > 0 && !catCols.includes('order_index')) {
        db_1.default.exec("ALTER TABLE categories ADD COLUMN order_index INTEGER DEFAULT 0");
        console.log('🔹 เพิ่มคอลัมน์ order_index ในตารางหมวดหมู่ย่อย (Categories) เรียบร้อย');
    }
    if (catCols.includes('is_fixed')) {
        try {
            db_1.default.exec("ALTER TABLE categories DROP COLUMN is_fixed");
...
const verifyGroupColumns = () => {
    ...
    if (groupCols.length > 0 && !groupCols.includes('order_index')) {
        db_1.default.exec("ALTER TABLE cashflow_groups ADD COLUMN order_index INTEGER DEFAULT 0");
    ...
    if (groupCols.length > 0 && !groupCols.includes('highlight_bg')) {
        db_1.default.exec("ALTER TABLE cashflow_groups ADD COLUMN highlight_bg INTEGER DEFAULT 0");
...
const verifyDayTypeColumns = () => {
    ...
    if (dayTypeCols.length > 0 && !dayTypeCols.includes('name')) {
        db_1.default.exec("ALTER TABLE day_types ADD COLUMN name TEXT DEFAULT ''");
...
const seedInitialData = () => {
    ...
    const insertDayType = db_1.default.prepare("INSERT INTO day_types (id, name, label, color, order_index) VALUES (?, ?, ?, ?, ?)");
