...
const generateMockupData = () => {
    ...
    const categories = db.prepare("SELECT c.id as cat_id, c.name as cat_name, cg.name as group_name, cg.type as group_type FROM categories c JOIN cashflow_groups cg ON c.cashflow_group_id = cg.id").all();
    ...
    const dayTypes = db.prepare("SELECT id, name FROM day_types").all();
    const dtMap = {
    ...
    const performBulkInsert = db.transaction((txs, cals) => {
        for (const tx of txs) {
