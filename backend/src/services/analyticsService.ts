...
class AnalyticsService {
   ...
   */
  private fetchMonthlyTotals(startDate?: string, endDate?: string) {
    const params: string[] = [];
    let query = `
      SELECT 
  ...
  private fetchMonthlyGroupTotals(startDate?: string, endDate?: string) {
  ...
  getMonthlyAggregation(startDate?: string, endDate?: string): MonthlyAggregationItem[] {
    const rows = this.fetchMonthlyTotals(startDate, endDate);
    const groupRows = this.fetchMonthlyGroupTotals(startDate, endDate);
